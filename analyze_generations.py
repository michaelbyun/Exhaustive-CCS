...
def main():
    ...
    prompt_num = 0
    last_labels = None
    while os.path.exists(f"./generation_results/log_probs/roberta-large-mnli_{dataset}_1000_prompt{prompt_num}_normal__log_probs.csv"):
        # load our shuffled data
        test_data_path = f"./generation_results/log_probs_test_sorted/roberta-large-mnli_{dataset}_1000_prompt{prompt_num}_normal__log_probs_test_sorted.csv"
        test_data = pd.read_csv(test_data_path)
        print(f"read test data from {test_data_path}")
        train_data_path = f"./generation_results/log_probs_train_sorted/roberta-large-mnli_{dataset}_1000_prompt{prompt_num}_normal__log_probs_train_sorted.csv"
